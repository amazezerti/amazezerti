...
I'm an Information Management graduate. I build enterprise software that helps organizations run their day-to-day operations, things like inventory, warehouses, finance, and business workflows, more effectively.

I like turning business requirements into scalable software solutions, with a strong focus on system architecture, database design, automation, inventory management, digital transformation, and enterprise workflows. My goal is to build technology that improves how organizations manage information, resources, and decision-making.

Right now I'm looking for international opportunities as a **Junior Software Developer**, **Information Systems Analyst**, **Business Systems Analyst**, or **IT Systems Officer**.

---

## What Makes Me Different

I like building software for real organizations. My projects span retail (a multi-branch inventory ERP), healthcare (a hospital lab management system), national government (an exam board's candidate-to-certificate pipeline), and commerce (an e-commerce platform). Each one solves an actual operational problem for an organization that runs on it.

I design business workflows before I write code, because understanding how an organization actually operates matters just as much as implementing the software for it.
...
A full-stack enterprise ERP I built for a real multi-branch retail/wholesale business. It covers the full operational loop: multi-branch and multi-warehouse inventory down to zone/rack/bin level, a POS-style sales workflow, procurement (purchase orders, goods-received notes, supplier returns), logistics and stock transfers, and accounting (expenses, profit & loss, KPI targets).
...
- Built a customer credit management workflow: customers are captured automatically from sales, with configurable credit limits and a branch-scoped approval and settlement process
...
- Automated business alerts (low stock, expiry dates, delayed transfers, expense thresholds) through scheduled background tasks, with branded PDF/Excel/CSV report exports
...
A large-scale information system digitizing the Republic of Chad's national baccalauréat (BAC) exam administration from end to end: candidate registration, region-scoped centre and seat assignment, double-blind grading, jury deliberation, results publication, and tamper-evident certificate issuance with public QR-code verification.
...
- Signed certificates with HMAC-SHA256 and verified them at a public, rate-limited endpoint that returns zero personal data, just pass/mention/serial metadata
- Enforced a hard, no-bypass registration deadline and an immutable audit log (a PostgreSQL trigger rejects any UPDATE or DELETE on it), plus mandatory TOTP 2FA for every high-privilege role and Fernet-encrypted candidate data at rest
- Shipped a full observability and deployment stack: Prometheus and Grafana, Celery/Redis for bulk uploads and scheduled encrypted backups, MinIO object storage, Nginx, and a GitHub Actions CI pipeline (flake8, black, bandit, pip-audit, pytest)
- 189 automated tests, 0 failures, across both the Django backend and the React/TypeScript frontend
...
- Modeled the complete clinical pipeline end to end: a Receptionist registers the patient, they get assigned to a Doctor load-balanced by department, a lab test order is created and confirmed, a Lab Technician collects the sample and enters results, the Doctor verifies them, and the Receptionist prints the report and processes payment
...
A full-stack e-commerce website I built and deployed for Société Ma-Moussa SARL, a real business in N'Djamena, Chad.

- Built a hardened admin panel behind a secret, rate-limited, code-split URL that's absent from navigation, source comments, and `robots.txt`, so it never appears in the public JS bundle
- Implemented defense-in-depth security: httpOnly + Secure + SameSite=Strict JWT cookies, bcrypt with 12 salt rounds, Helmet.js headers, strict CORS, tiered rate limiting (a global cap plus a much tighter one on login), input sanitization, and parameterized SQL throughout
- Uploaded images stream directly to Cloudinary with server-side MIME and size validation, so they never touch the server's disk
- Designed for a clean, environment-driven deploy: PostgreSQL locally, a one-line switch to Supabase in production, with zero code changes required
...
⚡ I believe understanding a business process matters just as much as writing the software for it
